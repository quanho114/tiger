-- ==============================================================================
-- Migration: 20260919000004_order_engine_and_quotes.sql
-- Description: Trusted Order Engine with atomic PostgreSQL RPC `create_order`
-- Enforces:
-- 1. Lock ordering: settings -> table/visit -> zone -> menu_items (sorted) -> orders
...
-- 4. Verification of visit capability context (active table, open visit, epoch)
-- 5. Strict price matching against live menu (raises QUOTE_CHANGED if altered)
-- 6. Atomic order, items, status history, guest claim and receipt persistence
-- ==============================================================================
...
BEGIN
    ...
    SELECT accepting_orders, accepting_dine_in_orders, accepting_delivery_orders
    INTO v_settings
    ...
    -- 4. Lock Ordering: Step B -> Lock table / visit if dine-in
    ...
    ELSIF p_order_type = 'delivery' THEN
        RAISE EXCEPTION 'UNSUPPORTED_ORDER_TYPE: Đặt hàng giao tận nơi chưa được hỗ trợ trong phiên bản này'
            USING ERRCODE = 'P0015';
    ELSE
    ...
    FOR v_item_idx IN 0 .. jsonb_array_length(p_items) - 1 LOOP
        ...
        SELECT id, name, price_vnd, published, available, allow_dine_in
        INTO v_item_record
        FROM public.menu_items
        WHERE id = v_item_id;
    ...
    INSERT INTO public.audit_logs (
        ...
        jsonb_build_object(
            ...
            'order_type', 'dine_in',
            'table_id', v_table_id,
            'table_visit_id', v_table_visit_id,
            'total_vnd', v_total_vnd,
