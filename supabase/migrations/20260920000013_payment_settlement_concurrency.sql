-- ==============================================================================
-- Migration 20260920000013: Payment, Visit & Order Concurrency Lock Ordering (F08 Remediation)
--
-- Fact-Forcing Metadata:
-- - Importers/Callers:
--   - Edge Functions: admin-api (POST /visits/:id/settle, POST /visits/:id/close, POST /orders/:id/payment, POST /orders/:id/transition)
--   - tests/integration/payments.test.ts, tests/integration/payment-concurrency.test.ts
-- - Affected RPCs:
...
-- - Data Schemas:
--   - public.table_visits (id, table_id, status, version, updated_at)
--   - public.orders (id, table_visit_id, status, payment_status, version, total_vnd, updated_at)
...
-- ------------------------------------------------------------------------------
-- 1. SETTLE TABLE VISIT PAYMENT (Lock rows before validation)
-- ------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.settle_table_visit_payment(
    p_visit_id uuid,
...
BEGIN
    ...
    -- 3. Lock Ordering:
    -- Step A -> Lock table_visits row FOR UPDATE (Parent)
    SELECT * INTO v_visit
    ...
    -- Step B -> Lock ALL order rows for this visit in deterministic order (id ASC)
    -- CRITICAL F08 FIX: Lock rows BEFORE validating counts, statuses, or expected versions!
    -- This prevents concurrent transactions from mutating orders between validation and lock.
    PERFORM 1
    FROM public.orders
    ...
    -- All rows are already locked FOR UPDATE above
    FOR v_order IN
        SELECT *
        FROM public.orders
        WHERE table_visit_id = p_visit_id
...
-- ------------------------------------------------------------------------------
-- 2. CLOSE TABLE VISIT (Lock rows before validation)
-- ------------------------------------------------------------------------------
...
DECLARE
    ...
    v_updated_visit public.table_visits%ROWTYPE;
BEGIN
    -- 1. Lock visit row FOR UPDATE (Parent)
    SELECT * INTO v_visit
    FROM public.table_visits
    WHERE id = p_visit_id
    FOR UPDATE;
    ...
    -- Step B -> Lock ALL order rows for this visit in deterministic order (id ASC)
    -- CRITICAL F08 FIX: Lock rows BEFORE validating terminal states and unpaid counts!
    PERFORM 1
    FROM public.orders
    WHERE table_visit_id = p_visit_id
    ORDER BY id ASC
    ...
    -- 4. Validate no unrefunded paid orders that were cancelled/rejected without refund
    ...
    IF v_unrefunded_paid_count > 0 THEN
        RAISE EXCEPTION 'PAID_ORDER_NOT_REFUNDED: Có % đơn hàng đã thanh toán nhưng bị hủy/từ chối chưa được xử lý hoàn tiền', v_unrefunded_paid_count
            USING ERRCODE = 'P0021';
    END IF;

    -- 5. Close visit, bump capability epoch to invalidate QR tokens, set closed_at
    UPDATE public.table_visits
    SET status = 'closed',
        closed_at = now(),
    ...
    INSERT INTO public.audit_logs (
        ...
        p_visit_id,
        jsonb_build_object(
            'old_data', jsonb_build_object('status', 'open', 'version', v_visit.version),
            'new_data', jsonb_build_object(
                'status', 'closed',
                'version', v_updated_visit.version,
                'capability_epoch', v_updated_visit.capability_epoch,
                'closed_at', v_updated_visit.closed_at
            )
        ),
        now()
    );
    ...
    RETURN jsonb_build_object(
        'id', v_updated_visit.id,
        'table_id', v_updated_visit.table_id,
        'status', v_updated_visit.status,
        'capability_epoch', v_updated_visit.capability_epoch,
        'version', v_updated_visit.version,
        'closed_at', v_updated_visit.closed_at
    );
...
-- ------------------------------------------------------------------------------
-- 3. RECORD ORDER PAYMENT (Uniform Lock Hierarchy: table_visits -> orders)
-- ------------------------------------------------------------------------------
...
BEGIN
    ...
    -- 3. Uniform Lock Hierarchy:
    -- Step A -> If this order belongs to a table visit, lock table_visits FOR SHARE first
    SELECT table_visit_id INTO v_table_visit_id
    FROM public.orders
    WHERE id = p_order_id;

    IF v_table_visit_id IS NOT NULL THEN
        PERFORM 1
        FROM public.table_visits
        WHERE id = v_table_visit_id
        FOR SHARE;
    END IF;

    -- Step B -> Lock order row FOR UPDATE
    SELECT * INTO v_order
    FROM public.orders
    WHERE id = p_order_id
    FOR UPDATE;
    ...
    INSERT INTO public.audit_logs (
        ...
        jsonb_build_object(
            ...
            'new_data', jsonb_build_object(
                ...
                'order_version', v_updated.version,
                'event_id', v_event_id
            ),
...
-- ------------------------------------------------------------------------------
-- 4. TRANSITION ORDER STATUS (Uniform Lock Hierarchy: table_visits -> orders)
-- ------------------------------------------------------------------------------
...
BEGIN
    ...
    -- Step B -> Lock the order row FOR UPDATE
    SELECT *
    INTO v_order
    FROM public.orders
    WHERE id = p_order_id
    ...
    ELSIF v_order.status = 'preparing' THEN
        ...
        ELSIF p_target_status = 'cancelled' THEN
            v_is_valid_transition := true;
            IF length(v_clean_reason) = 0 THEN
                v_clean_reason := 'Quán hủy đơn trong lúc đang chuẩn bị món';
            END IF;
        END IF;
    ...
    -- 6. Apply state transition with optimistic concurrency check
    UPDATE public.orders
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'VERSION_CONFLICT: Đơn hàng đã bị thay đổi bởi người khác'
            USING ERRCODE = 'P0003';
    END IF;

    -- 7. Insert into audit trail table (order_status_history)
